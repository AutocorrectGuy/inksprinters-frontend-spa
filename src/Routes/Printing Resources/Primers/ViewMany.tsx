...
const LatestPrimers = () => {
  ...
  const onRowClick = (primerId: number) =>
    navigate(PATH_CONSTANTS.PRINTING_RESOURCES.PRIMERS.VIEW_SINGLE + `?id=${primerId}`);

  const ToolbarTab = () =>
    <div className="flex justify-end p-4">
      <button className="btn btn-primary" onClick={() => navigate(PATH_CONSTANTS.PRINTING_RESOURCES.PRIMERS.CREATE)}>
        Add Primer
      </button>
    </div>
