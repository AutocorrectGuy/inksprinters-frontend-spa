...
const Breadcrumbs = ({
  ...
  setCurrentStep,
  resetComponentState,
  displayQueryParam }: BreadCrumbProps) => {
  const [searchParams] = useSearchParams();
  const [queriedValue, _setQueriedValue] = useState<string | null>(displayQueryParam ? searchParams.get(displayQueryParam) : null)
  ...
  const renderCrumbName = (href: string, i: number) => {
    ...
    if (!displayQueryParam || !queriedValue)
      return defaultText

    if (i === (breadcrumbPaths.length - 1))
      return `${defaultText}: ${toProperCase(queriedValue)}`

    return defaultText
  }

  return (
    <div className="text-lg breadcrumbs font-semibold text-[#CFCBC4] overflow-x-auto overflow-y-hidden"
      ...
      <ul>
        ...
        ))}
        {
          additionalSteps && additionalSteps?.map((step, i) =>
            <li key={`additional-crumb-link-${i}`}
