...
const AddManually = () => {
  const [formData, setFormData] = useState<Article>({ name: '', priming_duration: 0, created_at: 0 });
  const [_loadedData, setLoadedData] = useState<LoadedData>({ jigs: [], primers: [] })
  ...
  const handleSubmit = async () => {
    try {
      ...
      navigate(PATH_CONSTANTS.STORAGE.ARTICLES.MENU); // Redirect to viewMany component
    } catch (error) {
    ...
    <MainContentContainer h1='Add article' linkBackTo={PATH_CONSTANTS.STORAGE.ARTICLES.ADD.MENU}>
      <div className='flex flex-col grow items-center justify-center'>
        <form onSubmit={(e) => e.preventDefault()} className='grid grid-cols-2 gap-x-8 gap-y-2'>
          ...
          <div className="form-control">
            <span className='label-text'>{`Priming duration (in seconds)`}</span>
            <input type="text" name="priming_duration" className="input input-bordered" value={formData.priming_duration ?? ''} onChange={handleInputChange} />
          </div>
        ...
        <div className="flex w-full justify-center py-2">
          <button type="button" className="btn btn-primary text-lg" onClick={() => handleSubmit()}>Add</button>
        </div>
      </div>
